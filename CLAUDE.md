...
## Commands
- `python3 -m pytest tests/ -v` - run all tests (45 tests)
...
- `--mode ai`: Stage 3 uses `image_generator` (DALL-E 3), caches to `image_manifest.json`
...
- External APIs (Whisper, Claude, Pexels, OpenAI/DALL-E, MoviePy) are fully mocked in tests
...
- Image generator tests need `OPENAI_API_KEY` env var set via `@patch.dict(os.environ)` and mock both `openai` and `requests` at module level
