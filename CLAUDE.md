...
- Audio analyzer: Whisper transcribe uses `language="pl"`, `initial_prompt` (Polish Christian vocabulary), `temperature=0.0`, `condition_on_previous_text=True`; default model is "small" (not "base"); segment filtering skips segments with stripped text < 2 chars; analysis dict includes `energy_peaks` key — `list[float]` of onset strength peak times computed via `librosa.onset.onset_strength` + `librosa.util.peak_pick` + `librosa.frames_to_time`; `analyze_audio()` accepts `lyrics_path=None` — when provided, calls `merge_whisper_with_lyrics_file` internally (deferred import) so returned `lyrics` already contain file text with Whisper timing; saves to `audio_analysis.json` (not `analysis.json`)
...
- `--lyrics PATH`: custom .txt lyrics file (optional); auto-detects single .txt in audio dir. When provided, `lyrics_path` is passed to `analyze_audio()` which runs Whisper for timing then calls `merge_whisper_with_lyrics_file` internally (no Claude API call for alignment); CLI no longer calls merge directly
...
- CLI tests with `--lyrics`: merge now happens inside `analyze_audio()` (which is mocked in CLI tests), so no separate `merge_whisper_with_lyrics_file` mock needed; verify `lyrics_path` kwarg passed to `mock_analyze` instead
