...
## Commands
- `python3 -m pytest tests/ -v` - run all tests (~640 tests)
...
- WOW effects: `musicvid/pipeline/wow_effects.py` — FFmpeg post-processing applied after MoviePy writes output; `default_wow_config()` → dict with keys `enabled`, `zoom_punch`, `light_flash`, `dynamic_grade`, `dynamic_vignette`, `motion_blur`, `particles`; `build_ffmpeg_filter_chain(analysis, scene_plan, wow_config, video_width, video_height)` → str or None; `apply_wow_effects(video_path, ...)` runs FFmpeg in-place (temp file swap); subprocess mock target: `@patch("musicvid.pipeline.wow_effects.subprocess")`
- WOW filter builders: `_build_zoom_punch_filter(beats, sections, video_width, video_height)` — scale+crop, 8% zoom on chorus downbeats (every 4th beat, every 2nd selected), zoom 1.0→1.08 over 0.1s then 1.08→1.0 over 0.3s; `_build_light_flash_filter(sections)` — geq brightness spike at chorus starts decaying over 0.3s; `_build_color_grade_filter(sections)` — eq+colorbalance with `enable=` expressions (chorus: saturation 1.15/contrast 1.15, verse: saturation 0.85/contrast 1.05); `_build_vignette_filter(sections)` — vignette `enable=` per section (chorus angle=0.3, verse angle=0.6); motion blur via `tblend=all_mode=average:all_opacity=0.15`
